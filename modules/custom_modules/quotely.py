...
@Client.on_message(filters.command(["sq", "sq"], prefix))
@with_reply
async def uquote_cmd(client: Client, message: Message):
    ...
    if send_for_me:
        ...
        mm = await message.reply_text("<b>Generating...</b>")
    ...
    try:
        ...
        await mm.edit(format_exc(e))
    else:
        await mm.delete()


@Client.on_message(filters.command(["fs", "fsq"], prefix))
@with_reply
async def ufake_quote_cmd(client: Client, message: types.Message):
    is_png = "!png" in message.command or "!file" in message.command
    send_for_me = "!me" in message.command or "!ls" in message.command
    no_reply = "!noreply" in message.command or "!nr" in message.command
    ...
    if not fake_quote_text:
        return await message.reply_text("<b>Fake quote text is empty</b>")
    ...
    response = requests.post(url, json=params)
    if not response.ok:
        return await mm.edit(f"<b>Quotes API error!</b>\n<code>{response.text}</code>")
...
modules_help["uquotes"] = {
    "uq [reply]* [count 1-15] [!png] [!me] [!noreply]": "Generate a quote\n"
    "Available options: !png — send as PNG, !me — send quote to"
    "saved messages, !noreply — generate quote without reply",
    "ufq [reply]* [!png] [!me] [!noreply] [text]*": "Generate a fake quote",
}
